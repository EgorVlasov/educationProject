...
int main (){
    ...
    if (clientBirthYear > 1900 && clientBirthYear < currentYear){
        if (clientBirthMonth >= 1 && clientBirthMonth <= 12) {
            ...
            std::cout << "неверный месяц. Введите значение от 1 до 12";
        }
    } else {
        std::cout << "неверный год. Вы не могли родиться раньше 1900 года (столько не живут) и вы не похожи на путешественника во времени, чтобы прилететь из будущего.";
    }
}